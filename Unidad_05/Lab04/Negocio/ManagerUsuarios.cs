...
namespace Negocio
{
    public class ManagerUsuarios
    {
        ...
        public Usuario GetUsuario(int idUsuario)
        {
            ...
            SqlCommand cmdGetUsuario = new SqlCommand("select * from usuarios where id=@id", this.Conn);
            cmdGetUsuario.Parameters.Add("@id", SqlDbType.Int).Value = idUsuario;
            this.Conn.Open();
            SqlDataReader rdrUsuarios = cmdGetUsuario.ExecuteReader();
            if (rdrUsuarios.Read())
            {
                user.Id = (int)rdrUsuarios["id"];
                user.TipoDoc = (Nullable<int>)rdrUsuarios["tipo_doc"];
                user.NroDoc = (Nullable<int>)rdrUsuarios["nro_doc"];
                user.FechaNac = rdrUsuarios["fecha_nac"].ToString();
                user.Apellido = rdrUsuarios["apellido"].ToString();
                user.Nombre = rdrUsuarios["nombre"].ToString();
                user.Direccion = rdrUsuarios["direccion"].ToString();
                user.Telefono = rdrUsuarios["telefono"].ToString();
                user.Email = rdrUsuarios["email"].ToString();
                user.Celular = rdrUsuarios["celular"].ToString();
                user.NombreUsuario = rdrUsuarios["usuario"].ToString();
                user.Clave = rdrUsuarios["clave"].ToString();
            }
            this.Conn.Close();
            return user;
        }
